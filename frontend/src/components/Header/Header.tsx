...
function Header() {
  ...
  const handleNavClick = (event: React.MouseEvent, path: string, opts: { close?: boolean } = {}) => {
    ...
    <header className={`${styles.header} ${!isVisible ? styles.hidden : ''}`}>
      ...
      <div className={styles.container}>
        ...
        <div className={styles.headerActions}>
          ...
          {isAuthenticated && (
            <div className={styles.authUser}>
              <Link to="/perfil" onClick={(e) => handleNavClick(e, '/perfil')} aria-label="Ir a perfil">
                {user && <UserAvatar username={user.username} avatar={user.avatar} size={36} />}
              </Link>
              <button
                className={styles.logoutGhost}
                onClick={handleLogout}
                aria-label="Cerrar sesión"
                title="Cerrar sesión"
              >
                <svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor" xmlns="http://www.w3.org/2000/svg" aria-hidden>
        ...
        <MobileDrawer open={drawerOpen} onClose={closeDrawer}>
          {isAuthenticated ? (
            <div className={stylesMobile.userRow}>
              <div className={stylesMobile.userAvatar}>
                {user && (
                  <UserAvatar username={user.username} avatar={user.avatar} size={44} />
                )}
              </div>
              <div>
          ...
          <Link to="/" onClick={(event) => handleNavClick(event, '/', { close: true })} className={linkClass('/')}>
          ...
          {isAuthenticated && (
            <Link to="/perfil" onClick={(event) => handleNavClick(event, '/perfil', { close: true })} className={linkClass('/perfil')}>
              <span className={stylesMobile.linkIcon} aria-hidden><svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor" xmlns="http://www.w3.org/2000/svg"><path d="M12 12a4 4 0 100-8 4 4 0 000 8zm0 2c-5 0-8 2.5-8 4.5V22h16v-3.5c0-2-3-4.5-8-4.5z"/></svg></span>
              <span>Perfil</span>
            </Link>
          )}

          {isAuthenticated && user && (user.role === 'admin' || user.role === 'owner') && (
            <Link to="/admin/users" onClick={(event) => handleNavClick(event, '/admin/users', { close: true })} className={linkClass('/admin/users')}>
              <span className={stylesMobile.linkIcon} aria-hidden><svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor" xmlns="http://www.w3.org/2000/svg"><path d="M3 13h18v8H3zM3 3h18v8H3z"/></svg></span>
              <span>Admin</span>
            </Link>
          )}

          {!isAuthenticated && (
            <Link to="/login" onClick={(event) => handleNavClick(event, '/login', { close: true })} className={linkClass('/login')}>
              <span className={stylesMobile.linkIcon} aria-hidden><svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor" xmlns="http://www.w3.org/2000/svg"><path d="M12 2v6l4-3-4-3z"/></svg></span>
              <span>Login</span>
            </Link>
          )}

          {!isAuthenticated && (
            <Link to="/register" onClick={(event) => handleNavClick(event, '/register', { close: true })} className={linkClass('/register')}>
              <span className={stylesMobile.linkIcon} aria-hidden><svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor" xmlns="http://www.w3.org/2000/svg"><path d="M12 12c2.7 0 5-2.3 5-5s-2.3-5-5-5-5 2.3-5 5 2.3 5 5 5zM4 20c0-3.3 4.4-6 8-6s8 2.7 8 6v2H4v-2z"/></svg></span>
              <span>Registro</span>
            </Link>
          )}

          {isAuthenticated && (
            <button onClick={() => { closeDrawer(); handleLogout() }} className={styles.logoutBtn}>
              <span className={stylesMobile.linkIcon} aria-hidden><svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor" xmlns="http://www.w3.org/2000/svg"><path d="M16 13v-2H7V8l-5 4 5 4v-3z"/></svg></span>
              <span>Cerrar sesión</span>
            </button>
          )}

          <div className={stylesMobile.themeRow}>
            <span>Tema</span>
            <ThemeSwitch />
          </div>
        </MobileDrawer>
