...
export default function ResourceForm() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!user) return toast.error('Inicia sesión para subir recursos')
    setLoading(true)
    try {
      ...
      if (resolvedResourceId) {
        await updateResource(resolvedResourceId, formData)
      ...
      console.error(err)
      const data = (err as { data?: { errors?: Array<{ msg?: string; message?: string }>; error?: string }; message?: string }).data
    ...
    <div className={`${styles.root} ${styles.containerNarrow}`}>
      ...
      <form onSubmit={handleSubmit} className={styles.form}>
        ...
        <div className={styles.field}>
          ...
          <input className={styles.input} type="file" onChange={event => setFile(event.target.files?.[0] ?? null)} />
        </div>
