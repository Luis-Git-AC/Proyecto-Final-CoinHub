...
export default function CommentItem({ comment, onUpdated, onDeleted }: CommentItemProps) {
  const { user } = useAuth()
  ...
  const handleSave = async () => {
    if (!user) { toast.error('No autorizado'); return }
    if (!text.trim()) { toast.error('El comentario no puede estar vacío'); return }
    setLoading(true)
    try {
