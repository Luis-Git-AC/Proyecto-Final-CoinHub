...
router.get('/', auth, checkRole('admin', 'owner'), async (req, res) => {
  try {
...
router.delete('/:userId', auth, checkRole('admin', 'owner'), async (req, res) => {
  try {
    const userId = req.params.userId || req.params.id;
    const user = await User.findById(userId);
...
router.put('/:userId/role', auth, checkRole('admin', 'owner'), async (req, res) => {
  try {
    ...
    if (user.role === 'owner') {
      return res.status(403).json({ error: 'No está permitido cambiar el rol de un owner desde la administración. Use el script de mantenimiento o contacte al administrador del sistema.' })
    }
