...
async function openResource(req, res) {
  try {
    const resourceId = req.params.resourceId || req.params.id;
    const resource = await Resource.findById(resourceId);
    if (!resource) return res.status(404).json({ error: 'Recurso no encontrado' });
...
async function createResource(req, res) {
  try {
    ...
    const newResource = new Resource({
      ...
      category,
      originalName:
        req.file && req.file.originalname ? req.file.originalname : undefined
    });
...
async function updateResource(req, res) {
  try {
    ...
    if (
      ...
      return res.status(403).json({ error: 'No tienes permisos para editar este recurso' });
    }
    ...
    if (req.file) {
      ...
      try {
        ...
        const resourceType =
          newType === 'pdf' ? 'raw' : newType === 'image' ? 'image' : 'auto';
        const uploadOpts = { use_filename: true, unique_filename: false };
        const result = await uploadToCloudinary(
          req.file.buffer,
          'cryptohub/resources',
        ...
        resource.originalName =
          req.file && req.file.originalname ? req.file.originalname : resource.originalName;
      } catch (cloudError) {
        console.error('Error al subir nuevo archivo:', cloudError);
        return res.status(500).json({ error: 'Error al subir el nuevo archivo' });
      }
    }
